    ...
    static function ultimos_registros() {
        ...
        foreach ($response["hits"]["hits"] as $r){
            ...
            if (!empty($r["_source"]['name'])){
                echo '<a href="https://dedalus.usp.br/F/?func=direct&doc_number='.$r['_id'].'" target="_blank"><h4 class="uk-comment-title uk-margin-remove">'.$r["_source"]['name'].'';
...
function load_itens_single ($sysno) {
    $xml = simplexml_load_file('https://dedalus.usp.br/X?op=item-data&base=USP01&doc_number='.$sysno.'');
