...
class facets {
        ...
        foreach ($response["aggregations"]["counts"]["buckets"] as $facets) {
            ...
            echo '<div uk-grid>
                    ...
                    <div class="uk-width-1-3" style="color:#444">
                        <a href="https://'.$_SERVER["SERVER_NAME"].$_SERVER["SCRIPT_NAME"].'?'.$_SERVER["QUERY_STRING"].'&search[]=+'.$field.'.keyword:&quot;'.$facets['key'].'&quot;"  title="E" style="color:#0040ff;font-size: 65%">E</a>
                        <a href="https://'.$_SERVER["SERVER_NAME"].$_SERVER["SCRIPT_NAME"].'?'.$_SERVER["QUERY_STRING"].'&search[]=-'.$field.'.keyword:&quot;'.$facets['key'].'&quot;" title="NÃO" style="color:#0040ff;font-size: 65%">NÃO</a>
                    ';
            if (isset($get_search)) {
                echo '<a href="https://'.$_SERVER["SERVER_NAME"].$_SERVER["SCRIPT_NAME"].'?'.$_SERVER["QUERY_STRING"].'&search[]=OR '.$field.'.keyword:&quot;'.$facets['key'].'&quot;" title="OU" style="color:#0040ff;font-size: 65%">OU</a>';
            } 
            echo '</div></div></li>';
    ...
    public function facet_range($field,$size,$nome_do_campo) {
        ...
        foreach ($response["aggregations"]["ranges"]["buckets"] as $facets) {
            ...
            echo '<input type="checkbox" name="'.$field.'[]" value="'.$facets['key'].'"><a href="https://'.$_SERVER["SERVER_NAME"].$_SERVER["SCRIPT_NAME"].'?'.$_SERVER["QUERY_STRING"].'&search[]=+metrics.'.$field.':&quot;'.$facets['key'].'&quot;">Intervalo '.$facets['key'].' ('.number_format($facets['doc_count'],0,',','.').')</a>';
            echo '</p>';
            echo '</li>';
